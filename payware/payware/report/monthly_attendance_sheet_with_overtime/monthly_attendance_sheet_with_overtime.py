...
def execute(filters=None):
	...
	att_map_overtime_normal = get_overtime_normal(conditions, filters)
	att_map_overtime_holidays = get_overtime_holidays(conditions, filters)
	#frappe.msgprint(str(att_map_overtime))
	...
	for emp in sorted(att_map):
		...
		total_p = total_a = total_l = total_overtime_normal = totla_overtime_holidays = 0
		for day in range(filters["total_days_in_month"]):
			status = att_map.get(emp).get(day + 1, "None")
			overtime_normal = att_map_overtime_normal.get(emp).get(day + 1, "0")
			overtime_holidays = att_map_overtime_holidays.get(emp).get(day + 1, "0")
			...
			total_overtime_normal += float(overtime_normal)
			totla_overtime_holidays += float(overtime_holidays)
			if status == "Present":
				total_p += 1
			elif status == "Absent":
				total_a += 1
		...
		row += [total_p, total_l, total_a ,total_overtime_normal, totla_overtime_holidays ]
...
def get_columns(filters):
	...
	columns += [_("Overtime Normal") + ":Float:80", _("Overtime Holidays") + ":Float:80"]
	return columns

def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		status, overtime_normal, overtime_holidays from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
...
def get_overtime_normal(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		status, overtime_normal, overtime_holidays from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
		conditions, filters, as_dict=1)

	att_map_overtime_normal = {}
	for d in attendance_list:
		# late_hours
		att_map_overtime_normal.setdefault(d.employee, frappe._dict()).setdefault(d.day_of_month, "")
		att_map_overtime_normal[d.employee][d.day_of_month] = d.overtime_normal
		
	return   att_map_overtime_normal



def get_overtime_holidays(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		status, overtime_normal, overtime_holidays from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
		conditions, filters, as_dict=1)

	att_map_overtime_holidays = {}
	for d in attendance_list:
		# overtime_hours
		#frappe.msgprint(str(d.overtime_hours))
		att_map_overtime_holidays.setdefault(d.employee, frappe._dict()).setdefault(d.day_of_month, "")
		att_map_overtime_holidays[d.employee][d.day_of_month] = d.overtime_holidays
	#frappe.msgprint(str(d.overtime_hours))
	
	return  att_map_overtime_holidays
